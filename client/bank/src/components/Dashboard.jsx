...
export default function Dashboard({ approved }) {
	...
	useEffect(() => {
		...
		fetch(`${url}/kyc/getapprovals`, {
			...
			.then(res => {
				if (res.success) {
					approved
						? setdata(res.message.approved)
						: setdata(res.message.pending);
					setisloading(false);
				} else {
					setdata([]);
					setisloading(false);
	...
	return (
		<div>
			<h1 className="text-gray-800 text-center p-4">{`Welcome to ${
				bankname === "A" ? "HDFC Bank" : "Bank of America"
			} Dashboard`}</h1>
			<h1 className="text-gray-800 text-center p-4">
				{approved ? "Approved Users" : "Pending Approvals"}
			</h1>
			<div>
				{data.length === undefined || data.length <= 0 ? (
					<h2 className="text-gray-600 text-center">No data available</h2>
				) : (
					data.map((val, k) => {
						return (
